const amqp = require('amqplib');
const config = require('config');


class RabbitMQ {
    constructor() {
        const rabbitConfig = config.remotes.rabbitmq;
        this.URL = `amqp://${rabbitConfig.username}:${rabbitConfig.password}@${rabbitConfig.host}`;
        this.queueName = rabbitConfig.queueName;
        this.exchangeName = rabbitConfig.exchangeName;
        this.exchangeStrategy = rabbitConfig.exchangeStrategy;
        this.key = rabbitConfig.key;
        this.consumerTag = rabbitConfig.consumerTag; 
        this.connection = null;
        this.channel = null;
    }

    async connect() {
        this.connection = await amqp.connect(this.URL);
    }

    async fanoutPublish(data) {
        this.channel = await this.connection.createChannel();
        this.exchange = this.channel.assertExchange(this.exchangeName, this.exchangeStrategy, {durable: false});
        this.channel.publish(this.exchangeName, this.queueName, Buffer.from(JSON.stringify(data)));
    }

    async fanoutConsume() {
        this.channel = await this.connection.createChannel();
        this.exchange = this.channel.assertExchange(this.exchangeName, this.exchangeStrategy, {durable: false});
        const rabbitQueue = await this.channel.assertQueue(this.queueName, {exclusive: true});
        this.channel.bindQueue(rabbitQueue.queue, this.exchangeName, this.key);
        this.channel.consume(rabbitQueue.queue, function(data) {
            console.log("data: ", data);
        }, { noAck: true, consumerTag: config.remotes.rabbitmq.consumerTag });
    }

    async directPublish(data) {
        this.channel = await this.connection.createChannel();
        await this.channel.assertExchange(this.exchangeName, this.exchangeStrategy);
        await this.channel.assertQueue(this.queueName);
        this.channel.bindQueue(this.queueName, this.exchangeName, this.key);
        this.channel.sendToQueue(this.queueName, Buffer.from(JSON.stringify(data)))
    }

    async directConsume() {
        this.channel = await this.connection.createChannel();
        this.channel.consume(this.queueName, (message)=>{
            const data = JSON.parse(message.content);
            console.log('data: ', data);
            channel.ack(message);
        })
    }

    close() {
        setTimeout(() => {
            this.connection.close();
            console.log('closed rabbitmq connection');
        }, 500);
    }
}

module.exports = RabbitMQ;
